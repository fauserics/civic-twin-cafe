# civic_twin_cafe_app.py
import streamlit as st, pandas as pd, numpy as np
from pathlib import Path
import matplotlib.pyplot as plt

st.set_page_config(page_title="Cafetería Quilmes | Civic Twin", layout="wide")

SVG_LOGO = """
<svg width="48" height="48" viewBox="0 0 64 64" fill="none"
     xmlns="http://www.w3.org/2000/svg"
     style="vertical-align:middle;margin-right:12px">
  <circle cx="24" cy="32" r="18" stroke="white" stroke-width="6" fill="none"/>
  <circle cx="40" cy="32" r="18" stroke="white" stroke-width="6" fill="none"/>
</svg>
"""

HEADER_CSS = """
<style>
@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700&display=swap');
.header-bar{background:linear-gradient(90deg,#14406b 0%,#1F4E79 100%);
            border-radius:12px;padding:10px 20px;margin-bottom:16px;
            display:flex;align-items:center;justify-content:space-between;}
.header-left{display:flex;align-items:center;gap:14px}
.proj-title{font:700 34px 'Montserrat',sans-serif;color:#fff;margin:0}
.proj-sub{font:400 20px 'Montserrat',sans-serif;color:#d0e1ff;margin:0 0 0 12px}
.flag{height:34px;border-radius:3px}
.stMetric>div{border:2px solid #1F4E79!important;border-radius:10px;
              background:#fff;box-shadow:0 2px 6px #00000014;padding:12px 8px}
input[type=range]::-webkit-slider-runnable-track{background:#1F4E7933}
input[type=range]::-webkit-slider-thumb{background:#1F4E79;border:none}
input[type=range]::-moz-range-track{background:#1F4E7933}
input[type=range]::-moz-range-thumb{background:#1F4E79;border:none}
section[data-testid=stSidebar]{background:#eaf0f7}
</style>
"""

FLAG_AR = "https://flagcdn.com/w40/ar.png"

header_html = (
f"{HEADER_CSS}"
"<div class='header-bar'>"
  "<div class='header-left'>"
    f"{SVG_LOGO}"
    "<span class='proj-title'>Cafetería Quilmes</span>"
    "<span class='proj-sub'>Civic Twin</span>"
  "</div>"
  f"<img src='{FLAG_AR}' class='flag'>"
"</div>"
)

st.markdown(header_html, unsafe_allow_html=True)

# ---------- (resto del código: carga de datos, sidebar, KPI, gráfico) ----------
BASE=Path(__file__).parent; CSV=BASE/'CivicTwin_Cafe_Quilmes_Data.csv'; XLSX=BASE/'CivicTwin_Cafe_Quilmes_Data.xlsx'
@st.cache_data
def load(): 
    if CSV.exists(): return {"tidy":pd.read_csv(CSV)}
    if XLSX.exists(): return pd.read_excel(XLSX,sheet_name=None)
    st.error("Falta dataset"); return {}
data=load(); 
if not data: st.stop()
if "tidy" in data:
    t=data["tidy"]; init=t[t.dataset=="initial_costs"]; month=t[t.dataset=="monthly_costs"]; 
    sales=t[t.dataset=="sales_scenarios"]; ass=t[t.dataset=="assumptions"]
else:
    init=data["initial_costs"]; month=data["monthly_costs"]; sales=data["sales_scenarios"]; ass=data["assumptions"]
assump=dict(zip(ass.variable,ass.value)); WD=int(assump.get("working_days_per_month",26))
PC=float(assump.get("insumos_percent_of_sales",0.30)); INV=init.cost_ars.sum(); FIX=month.cost_ars.sum()

st.sidebar.header("Escenario")
cli=st.sidebar.slider("Clientes por día",30,200,int(sales.loc[sales.scenario=="Moderado","clients_per_day"]),5)
tic=st.sidebar.slider("Ticket promedio (ARS)",3000,8000,int(sales.loc[sales.scenario=="Moderado","ticket_ars"]),100)
inf=st.sidebar.number_input("Inflación anual (%)",0.0,200.0,0.0,1.0)

vent=cli*tic*WD; ins=vent*PC; gan=vent-(ins+FIX); pay="∞" if gan<=0 else INV/gan
c1,c2,c3=st.columns(3); c1.metric("Ventas mensuales",f"${vent:,.0f}"); c2.metric("Ganancia mensual",f"${gan:,.0f}")
c3.metric("Pay-back (meses)","No rentable" if pay=="∞" else f"{pay:.1f}")
st.divider()
mes=np.arange(1,25); serie=gan*(1+inf/100)**(mes/12); flujo=np.cumsum(serie)-INV
fig,ax=plt.subplots(); ax.plot(mes,flujo,color="#1F4E79",lw=2); ax.axhline(0,color="#888",lw=.8,ls="--")
ax.set_xlabel("Mes"); ax.set_ylabel("Flujo acumulado (ARS)"); ax.set_title("Proyección 24 meses",color="#14406b",weight="bold")
st.pyplot(fig); st.caption("Datos: CivicTwin_Cafe_Quilmes_Data · Jul 2025")
