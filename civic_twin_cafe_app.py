...
# Versión mock-up 2025-07-07 (c) – header fix
...
.stMetric>div{border:2px solid #1F4E79!important;border-radius:10px;
    background:#ffffff;box-shadow:0 2px 6px #00000014;padding:12px 8px}
...
clients_day = st.sidebar.slider("Clientes por día",30,200,
    int(sales_df.loc[sales_df.scenario=="Moderado","clients_per_day"]),5)
ticket_avg  = st.sidebar.slider("Ticket promedio (ARS)",3000,8000,
    int(sales_df.loc[sales_df.scenario=="Moderado","ticket_ars"]),100)
inflacion   = st.sidebar.number_input("Inflación anual (%)",0.0,200.0,0.0,1.0)
...
c1.metric("Ventas mensuales",f"${ventas_m:,.0f}")
c2.metric("Ganancia mensual",f"${ganancia_m:,.0f}")
...
st.caption("Datos: CivicTwin_Cafe_Quilmes_Data · Jul 2025")
